...
struct MmaPipelineTest
{
                  ...
                  std::function<CType(uint32_t, ScaleAType, ScaleBType)> getExpected)
    {
        ...
        // Initialize A and B to all 1's, C to all 0's
        std::vector<AType> h_a(AElements, type_convert<AType, float>(1.0f));
        std::vector<BType> h_b(BElements, type_convert<BType, float>(1.0f));
        std::vector<CType> h_c(CElements, type_convert<CType, float>(0.0f));
        std::vector<CType> h_out(CElements, type_convert<CType, float>(0.0f));
