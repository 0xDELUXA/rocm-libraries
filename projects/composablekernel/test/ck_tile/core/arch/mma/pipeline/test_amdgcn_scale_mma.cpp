...
{
    using TestScaleMfma = amdgcn_mma<AType,
                                     BType,
                                     CType,
                                     WaveTileM,
                                     WaveTileN,
                                     WaveTileK,
                                     DefaultScaleMfmaCtrlFlags<AType, BType>,
                                     CompilerTargetGfx950,
                                     MmaOpFamily::SCALE>;

    static_assert(std::is_same_v<typename TestScaleMfma::OpType, MfmaOp> &&
                      TestScaleMfma::OpFamily == MmaOpFamily::SCALE,
                  "GFX950 scale 16x16x128 should have ScaleMFMAOp type");

    static_assert(is_mma_op_of_family_v<MmaOpFamily::SCALE, TestScaleMfma>,
...
template <typename AType,
          ...
          std::uint32_t WaveTileN,
          std::uint32_t WaveTileK>
void MmaOpTraitsIntegration_impl()
{
...
TEST(ScaleMMATrait, MmaOpTraitsIntegration)
{
    ...
    MmaOpTraitsIntegration_impl<fp8_t, fp8_t, fp32_t, 16u, 16u, 128u>();
    // Create a scale MMA op (16x16x128 bf8 specialization)
    MmaOpTraitsIntegration_impl<bf8_t, bf8_t, fp32_t, 16u, 16u, 128u>();
    // Create a scale MMA op (16x16x128 fp4 specialization)
    MmaOpTraitsIntegration_impl<pk_fp4_t, pk_fp4_t, fp32_t, 16u, 16u, 128u>();
    // Create a scale MMA op (32x32x64 fp8 specialization)
    MmaOpTraitsIntegration_impl<fp8_t, fp8_t, fp32_t, 32u, 32u, 64u>();
    // Create a scale MMA op (32x32x64 bf8 specialization)
    MmaOpTraitsIntegration_impl<bf8_t, bf8_t, fp32_t, 32u, 32u, 64u>();
    // Create a scale MMA op (32x32x64 fp4 specialization)
    MmaOpTraitsIntegration_impl<pk_fp4_t, pk_fp4_t, fp32_t, 32u, 32u, 64u>();
